        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
            <tr>
              ...
              <th className="text-left p-3 hidden sm:table-cell">Country</th>
              <th className="text-left p-3">KYC</th>
              <th className="text-right p-3 hidden sm:table-cell">Wallet</th>
          ...
          <tbody className="divide-y">
            {adminUsers.map((u) => (
              <tr key={u.email} className="hover:bg-muted/30">
                ...
                <td className="p-3 hidden sm:table-cell">{u.country}</td>
                <td className="p-3"><StatusBadge status={u.kyc} /></td>
                <td className="p-3 text-right hidden sm:table-cell">${u.wallet.toFixed(2)}</td>
